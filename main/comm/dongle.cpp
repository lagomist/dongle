...
constexpr static uint8_t const SCAN_MAX_BUF_NUM = 100;

struct ScanStats {
	uint32_t total_reports;
	uint32_t scan_response_reports;
	uint32_t extended_reports;
	uint32_t anonymous_reports;
...
static void scan_callback(Wrapper::BLE::Client::AdvReport report) {
	_scan_stats.total_reports++;
	if (report.scan_response) {
		_scan_stats.scan_response_reports++;
	}
	if (report.extended_pdu) {
		_scan_stats.extended_reports++;
	}
	if (report.anonymous) {
		_scan_stats.anonymous_reports++;
	}

	int index = scan_list_find_by_addr(report.peer_addr);
	bool new_device = (index < 0);
	if (index < 0) {
		index = scan_list_find_empty();
	}
	if (index < 0) {
		_scan_stats.dropped_reports++;
		if (new_device) {
			_scan_stats.dropped_unique_devices++;
		}
		return;
	}
	if (new_device) {
		_scan_stats.unique_devices++;
	}
...
static void dongle_handler(void *param) {
	...
	case Wrapper::BLE::Client::EvtType::SCAN_TIMEOUT_EVT:
		usb_cli::write("Scan summary: mode=%s, reports=%lu, unique=%lu, scan_rsp=%lu, ext=%lu, anon=%lu, dropped=%lu, dropped_unique=%lu, capacity=%u\n",
						...
						(unsigned long)_scan_stats.total_reports,
						(unsigned long)_scan_stats.unique_devices,
						(unsigned long)_scan_stats.scan_response_reports,
						(unsigned long)_scan_stats.extended_reports,
						(unsigned long)_scan_stats.anonymous_reports,
